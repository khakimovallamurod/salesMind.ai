...
export default function App() {
  ...
  useEffect(() => {
    ...
    const auth = localStorage.getItem('salesmind_auth');
    if (auth === 'true') {
  ...
  const handleLogin = () => {
    setIsAuthenticated(true);
    localStorage.setItem('salesmind_auth', 'true');
  };

  const handleLogout = () => {
    setIsAuthenticated(false);
    localStorage.removeItem('salesmind_auth');
  };
              ...
              <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <button 
                    ...
                    className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-rose-600 hover:bg-rose-50 transition-colors shadow-sm"
                  >
                    Logout
                  </button>
